...
0. Initial Program-Guided Toolchain Attempt:
...
1. Attempt with `riscv-collab` Nightly Build:
...
- For some toolchains, ISA sub-extensions (like `_zicsr`, `_zifencei`) must be explicitly added to the `-march` string for the assembler to recognize all standard instructions, even if the base ISA version implies them by specification. This was observed with the xPack toolchain for CSRs in assembly.
- xPack toolchains demonstrated robust multilib support for C code, which was lacking in the other attempts.
- The initial program-guided toolchain (`riscv-toolchain-rv32imac-x86_64-ubuntu.tar.gz`) was found to be fundamentally flawed in its CSR assembly capabilities for `rv32i/imac`.
